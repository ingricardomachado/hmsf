    ...
    <table class="table" width="100%" style="font-size:10px">
        <thead>
            <tr>
                ...
                <th class="text-left">Mensajero</th>
                <th class="text-left">Folio</th>
                <th class="text-right">Facturado</th>
                <th class="text-right">Margen Total</th>
                <th class="text-right">Margen SC</th>
                <th class="text-right">Margen HM</th>
        ...
        @foreach($operations as $operation)                    
            <tr>
                ...
                <td class="text-left" valign="top">{{ ($operation->user_id)?$operation->user->full_name:'' }}</td>
                <td class="text-left" valign="top">{{ $operation->folio }}</td>
                <td class="text-right" valign="top">{{ session('coin') }}{{ money_fmt($operation->amount) }}</td>
                <td class="text-right" valign="top">({{ money_fmt($operation->customer_tax) }}%) {{ session('coin') }}{{ money_fmt($operation->customer_profit) }}</td>
                <td class="text-right" valign="top">({{ money_fmt($operation->partner_tax) }}%) {{ session('coin') }}{{ money_fmt($operation->partner_profit) }}</td>
                <td class="text-right" valign="top">({{ money_fmt($operation->hm_tax) }}%) {{ session('coin') }}{{ money_fmt($operation->hm_profit) }}</td>
        ...
        <tfoot>
            <tr>
                ...
                <th></th>
                <th class="text-right">
                    {{ session('coin') }}{{ money_fmt($operations->sum('amount')) }}
                </th>
                <th class="text-right">
                    {{ session('coin') }}{{ money_fmt($operations->sum('customer_profit')) }}
                </th>
                <th class="text-right">
                    {{ session('coin') }}{{ money_fmt($operations->sum('partner_profit')) }}
                </th>
                <th class="text-right">
                    {{ session('coin') }}{{ money_fmt($operations->sum('hm_profit')) }}
                </th>
                <th class="text-right">
